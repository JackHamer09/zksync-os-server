...
/// Resolves the first L1 block at which L1's MessageRoot has emitted interop-root log id
/// `next_interop_root_id` (the chain's interop-root cursor). Returns block 0 for a zero cursor
/// (nothing imported yet). Since gateway settlement support was removed, interop roots are emitted
/// by L1's MessageRoot, so this anchors the single L1 interop-roots watcher's start block.
